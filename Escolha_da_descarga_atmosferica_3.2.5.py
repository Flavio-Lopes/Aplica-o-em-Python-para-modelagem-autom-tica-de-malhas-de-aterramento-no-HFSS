...
def modelo_1():
    ...
    diametro_hastes = float(input("\nInsira o raio das hastes entre 10-15 mm: \n").strip())
...
def lightning(tpp, tmm, tend, peak, resol, diretorio):
    ...
    np.savetxt(file_path, np.column_stack((t, I)), delimiter='\t', header='Time (µs)\tCurrent (kA)', comments='')

    return K, alpha, beta, emax, kk, file_path, file_name

#Função para definir uma descarga de 8/20us já disponível no arquivo base do HFSS para simulação; Retorna também o gráfico dessa forma de onda 
#para  o usuário
def define_input1():
...
#Função para definir uma descarga de 1.2/50us já disponível no arquivo base do HFSS para simulação; Retorna também o gráfico dessa forma de onda 
#para  o usuário
def define_input2():
    diretorio_raio=os.path.dirname(caminho_projeto_novo)
    nome_projeto_novo = os.path.splitext(os.path.basename(caminho_projeto_novo))[0] 
...
"""	
    ...
    # Criar um gráfico XY
    plt.figure(figsize=(10,6))
    plt.plot(df['X '], df['Y']*peak, marker='o')  # Agora você pode usar 'X' e 'Y' diretamente
...
#Função para definir a descarga personalizada do usuário
def define_input3():
    diretorio_raio = os.path.dirname(caminho_projeto_novo)
    while True:
...
#Função principal para desenvolvimento da aplicação, responsável por chamar as outras funções dado o avanço do usuário
def escolhe_modelo():
